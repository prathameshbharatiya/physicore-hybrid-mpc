...
interface SimulationCanvasProps {
  ...
  physicsPriors: { mass: number; friction: number; gravity: number; textile_k?: number; damping?: number };
}
...
const SimulationCanvas: React.FC<SimulationCanvasProps> = ({ mode, onStateUpdate, target, controlAction, physicsPriors }) => {
  ...
  useEffect(() => {
    ...
    const drawOverlay = () => {
      ...
      for (let i = 0; i < 20; i++) {
        tempX = stepDynamicsRK4(tempX, controlAction, physicsPriors as any);
