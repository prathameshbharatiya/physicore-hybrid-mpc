...
class PhysicoreSensorFilter:
    ...
    def __init__(
        ...
        self.altitude     = 0.0
        self.vz           = 0.0
        self.velocity_x   = 0.0  # SF-02: standardized name (was self.vx)
        self.velocity_y   = 0.0  # SF-02: standardized name (was self.vy)
        self.accel_x      = 0.0
        self.accel_y      = 0.0
        self.accel_z      = 0.0
        self.gyro_x       = 0.0
    ...
    def process(
        ...
        gps_vx: Optional[float] = None,
        gps_vy: Optional[float] = None,
        gps_vz: Optional[float] = None,  # SF-01: now actually passed through to VelocityTracker
        ...
        # SF-03: gravity correction on the ALIGNED az (Z-up world frame after frame.align_accel()).
        # Frame alignment must happen BEFORE gravity subtraction — do NOT subtract 9.81 from raw_az.
        # For z_down mounting: raw_az≈-9.81 → frame flips to +9.81 → vert_accel = +9.81 - 9.81 = 0 ✓
        # (Old code subtracted from already-aligned az, which was correct, but the ordering comment
        #  in the bug report was misleading — the actual fix is to ensure frame alignment is step 1.)
        vert_accel = az - 9.81  # az is already aligned (Z-up), so this is correct
        ...
        # SF-01: gps_vz now passed through — GPS vertical velocity correction was dead code
        vx, vy, vz_from_tracker = self.vel_tracker.update(
            net_ax, net_ay, 0.0,
            gps_vx=gps_vx,
            gps_vy=gps_vy,
            gps_vz=gps_vz,   # SF-01: this was always None before — now propagated from caller
            phase=self.phase,
        )
        ...
        self.velocity_x = vx; self.velocity_y = vy  # SF-02: standardized names

        return self

    # SF-02: backward-compat aliases — old bridge code using .vx/.vy still works
    @property
    def vx(self) -> float:
