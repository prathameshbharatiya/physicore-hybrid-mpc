...
class EngineState:
    def __init__(self):
        ...
        self.lock = asyncio.Lock()  # S-07: asyncio.Lock to avoid blocking event loop in async routes
...
async def lifespan(app: FastAPI):
    ...
    # S-08: start broadcast_diagnostics exactly once via create_task
    # (removed any duplicate invocations — only one entry point for this coroutine)
    _broadcast_task = asyncio.create_task(broadcast_diagnostics())
    yield
...
async def configure_engine(req: ConfigureRequest, api_key: str = Depends(require_api_key)):  # S-06
    ...
    async with engine_state.lock:  # S-07
...
async def engine_step(req: StepRequest, api_key: str = Depends(require_api_key)):  # S-06
...
async def engine_observe(req: ObserveRequest, api_key: str = Depends(require_api_key)):  # S-06
    engine = require_engine()
    async with engine_state.lock:  # S-07
        engine.observe(
            np.array(req.state),
...
async def reset_engine(api_key: str = Depends(require_api_key)):  # S-06
    async with engine_state.lock:  # S-07
...
async def ws_telemetry(websocket: WebSocket):
    ...
    try:
        # S-13: drain incoming messages concurrently so TCP buffer never fills
        # without this, clients sending any message cause the connection to stall
        async def _drain():
            try:
                async for _ in websocket.iter_text():
                    pass  # telemetry is send-only; discard any client messages
            except Exception:
                pass
...
async def registry_save(platform: str, api_key: str = Depends(require_api_key)):  # S-06
...
async def registry_sessions(platform: str, limit: int = 20):
    ...
    sessions = []
    with open(fp) as fh:  # use context manager to close file handle
        for line in fh:
            try:
                sessions.append(json.loads(line.strip()))  # S-05: use json not _json
            except Exception:
                pass
...
Analyze this and return JSON."""
    ...
    # S-11: privacy gate — only call external Gemini API if user opted in
    if not os.environ.get("PHYSICORE_AI_TELEMETRY_OPT_IN"):
        raw = f"{nar['headline']}\n\n{nar['detail']}\n\nRecommendation: {nar['action']}"
    else:
...
Give exact numbered steps to fix this."""
    ...
    # S-11: privacy gate — only call external Gemini API if user opted in
    if not os.environ.get("PHYSICORE_AI_TELEMETRY_OPT_IN"):
        # Fall back to local narration — never sends hardware data to Google
        if engine_state.engine:
            nar = engine_state.engine.narrate()
            raw = f"{nar['headline']}\n\n{nar['detail']}\n\nRecommendation: {nar['action']}"
        else:
            raw = "Engine not running."
    else:
        raw = _call_gemini(system_prompt, user_prompt)
...
# S-12: default to None — the api.physicore.ai server doesn't exist yet
# Only make network requests if PHYSICORE_SHARED_REGISTRY_URL is explicitly configured
SHARED_REGISTRY_URL = os.environ.get("PHYSICORE_SHARED_REGISTRY_URL", None)
...
async def get_shared_prior(platform: str):
    ...
    """
    # S-12: skip the request entirely if SHARED_REGISTRY_URL is not configured
    if not SHARED_REGISTRY_URL:
        return {"platform": platform, "params": {}, "sessions": 0,
                "source": "no_shared_registry"}
...
async def sentinel_configure(platform: str = "balancing_bot", api_key: str = Depends(require_api_key)):  # S-06
...
async def sentinel_step(req: SentinelStepRequest, api_key: str = Depends(require_api_key)):  # S-06
    ...
    try:
        ...
        if len(hist) >= 10 and res < 0.05:
            ...
            if max(last5) - min(last5) < 0.002:   # stable
                # S-09: only fire once — add flag so we don't flood at 20Hz forever
                if not getattr(engine, '_convergence_fired', False):
                    engine._convergence_fired = True
...
async def registry_convergence(platform: str):
    ...
    with open(fp) as f:
        for line in f:
            try:
                sessions.append(json.loads(line.strip()))  # S-05: use json not _json (which was never imported in this scope)
            except Exception:
                pass
    ...
    async def event_stream():
        ...
        while True:
            try:
                if engine_state.engine:
                    nar = engine_state.engine.narrate()
                    # S-10: always yield every second — remove status-change gate
                    # so live metric values (residual, mass, steps) always update in the IDE
